...
router.post('/apartments/:id/confirm-clean', async (req, res) => {
  ...
  const updated = db.prepare(`SELECT * FROM apartments WHERE id = ?`).get(apt.id);
  res.json({ apartment: updated, status: newStatus });
});
